...
begin
  if not exists (
    ...
    raise exception 'WidgetFlow migration 045_signature_self_service must be applied first';
  end if;
  ...
  if exists (
    ...
    raise exception 'WidgetFlow migration 046_signature_self_service_profile_identity_hotfix has already been applied';
  end if;
end
$guard$;
...
revoke all
on function public.get_my_signature_profile()
from public, anon, authenticated;

revoke all
on function public.save_my_signature_profile(jsonb)
from public, anon, authenticated;
...
values (
  '046_signature_self_service_profile_identity_hotfix',
  'Fix self-service signature profile identity lookup to match canonical profiles schema'
);
