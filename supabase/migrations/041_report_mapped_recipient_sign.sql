...
begin
  if not exists (
    ...
    raise exception
      'WidgetFlow migration 040_report_signature_mapping_send must be applied first';
  end if;
  ...
  if exists (
    ...
    raise exception
      'WidgetFlow migration 041_report_mapped_recipient_sign has already been applied';
  end if;
end
$guard$;
...
begin
  ...
  where discovered.field_key = mapping_row.signature_field_key;
...
revoke all
on function public.sign_report(
  uuid,
  uuid,
  jsonb
)
...
values (
  ...
  'Mapped recipient signature snapshot and signer-only report finalization'
);
